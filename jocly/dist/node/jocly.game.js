...
JocGame.prototype.Init = function (aOptions) {
  ...
  if (aOptions) {
    ...
    this.mVisitedBoards = {};
    if (typeof this.mOptions.viewAs != "undefined") this.mViewAs = this.mOptions.viewAs;else this.mOptions.viewAs = this.mViewAs;
  }
...
JocGame.prototype.RemoveListener = function (listener) {
  for (var i = this.listeners.length - 1; i >= 0; i--) if (this.listeners[i] == listener) this.listeners.splice(i, 1);
};
...
JocGame.prototype.AttachElement = function (element, options) {
  ...
  var promise = new Promise(function (resolve, reject) {
    if (game.gamePreAttachProto) reject(new Error("Game already attached"));else {
      ...
      Promise.all([window.BrowserScriptLoader["import"]("jquery.js"), window.BrowserScriptLoader["import"]("three.js").then(function (threeExports) {
        ...
        if (typeof window.THREE === "undefined") window.THREE = threeExports;
        ...
        if (defaultViewOptions) {
          ...
          for (var opt in optDefs) if (typeof defaultViewOptions[optDefs[opt]] != "undefined") game[opt] = defaultViewOptions[optDefs[opt]];
        }
        game.UpdateSounds();
        resolve();
      }, function (e) {
...
JocGame.prototype.UpdateSounds = function () {
  ...
  if (joclySounds.length == 0) joclySounds = $("<div/>").attr("id", "jocly-sounds").css({
    display: "none"
  }).appendTo($("body"));
  ...
  for (var i in defaultSounds) AddSound(i, this.config.baseURL, defaultSounds[i]);
...
JocGame.prototype.StartMachine = function (aOptions) {
  ...
  if (aOptions.level && aOptions.level.ai == "fairy-stockfish" && typeof JoclyFairy != "undefined") {
    ...
    if (aiThread) this.StartThreadedMachine(aOptions, "alpha-beta");else {
      ...
      for (var s in this.mVisitedBoards) this.mSavedVisitedBoards[s] = this.mVisitedBoards[s];
...
JocGame.prototype.StopThreadedMachine = function () {
  ...
  // move/level change.
  if (typeof JoclyFairy != "undefined" && JoclyFairy.abortMachine) JoclyFairy.abortMachine(this);
  // Same rationale for Scan (see jocly.scan.js).
  if (typeof JoclyScan != "undefined" && JoclyScan.abortMachine) JoclyScan.abortMachine(this);
};
...
JocGame.prototype.Engine = function (aBoard, aLevel, aBAlpha, aAlpha, aPotential) {
  ...
  if (typeof context.mBoard.mMoves == "undefined") context.mBoard.mMoves = [];
...
JocGame.prototype.HandleRepeat = function (board) {
  if (this.mOptions.preventRepeat) {
    ...
    if (this.mVisitedBoards[sign] === undefined) this.mVisitedBoards[sign] = 1 + 16 * this.mPlayedMoves.length;else this.mVisitedBoards[sign]++;
  }
};
JocGame.prototype.UnhandleRepeat = function (board) {
  if (this.mOptions.preventRepeat) {
    ...
    if (n == 1) delete this.mVisitedBoards[sign];else if (n > 1) this.mVisitedBoards[sign]--;
  }
};
...
JocGame.prototype.ExportInitialBoardState = function (format) {
  if (!this.mInitialString) return null;
  if (typeof this.Import != "function") return null;
  try {
    var importResult = this.Import("pjn", this.mInitialString);
    if (!importResult.status) return null;
    var board = new (this.GetBoardClass())(this);
    if (board.InitialPosition) board.InitialPosition(this);
...
JocGame.prototype.Load = function (gameData) {
  ...
  if (gameData.initialBoard) {
    if (typeof this.Import != "function") throw new Error("Import not supported");
    ...
    if (!importResult.status) {
      ...
      switch (importResult.error) {
        case 'parse':
          error = new Error("import failed: parse error");
          break;
        case 'unsupported':
          error = new Error("import failed: unsupported format");
          break;
      }
      throw error;
    }
    this.mInitial = importResult.initial;
    if (this.mInitial.turn) this.mWho = this.mInitial.turn;
...
JocBoard.prototype.GenerateMoveObjects = function (aGame) {
  ...
  this.mMoves = [];
  this.GenerateMoves(aGame);
  for (var i = 0; i < this.mMoves.length; i++) moves.push(aGame.CreateMove(this.mMoves[i]));
  this.mMoves = moves;
};
...
JocBoard.prototype.PickMoveFromDatabase = function (aGame, database) {
  ...
  var totalEval = 0;
  for (var i = 0; i < dbMoves.length; i++) totalEval += dbMoves[i].e;
...
JocGame.Zobrist = function (params) {
  ...
  var paramNames = [];
  for (var f in params) paramNames.push(f);
  ...
  for (var pi = 0; pi < paramNames.length; pi++) {
    ...
    var vIndex = 0;
    for (var vi = 0; vi < param.values.length; vi++) seed.values[param.values[vi]] = vIndex++;
    switch (param.type) {
      case "array":
        for (var j = 0; j < param.size; j++) {
          var seeds0 = [];
          for (var i = 0; i < vIndex; i++) seeds0.push(mt.genrand_int32());
          seed.seeds.push(seeds0);
        }
        seed.type = "array";
        break;
      default:
        for (var i = 0; i < vIndex; i++) seed.seeds.push(mt.genrand_int32());
        seed.type = "simple";
    }
    this.seed[f] = seed;
  }
...
JocGame.Levenshtein = function (e, f) {
  ...
  for (var b = Array(d + 1), g = Array(d + 1), a = 0, h = 0, i = 0, a = 0; a < d + 1; a++) b[a] = a;
