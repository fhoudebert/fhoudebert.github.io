  ...
  JoclyUCT.startMachine = function (aGame, aOptions) {
    ...
    function PropagateEvalParent(node, visits, visited) {
      ...
      if (uctParams.ignoreLeaf) {
        ...
        for (var i = 0; i < node.children.length; i++) if (node.children[i].n.children) {
      ...
      if (children.length == 0) for (var i = 0; i < node.children.length; i++) children.push(node.children[i].n);
      var evaluation;
      ...
      if (uctParams.uncertaintyFactor)
        // tend to do good things now rather than later
    ...
    function PropagateKnown(node, visited) {
      ...
      for (var i = 0; i < node.parents.length; i++) PropagateKnownParent(node.parents[i], visited);
    }
    ...
    function Step() {
      ...
      var _loop = function _loop() {
          ...
          if (node.children === null) return 0; // break
          if (uctParams.useAlphaBeta) {
            candidateChildren = [];
          ...
          if (aGame.mOptions.uctTransposition && !aGame.mOptions.uctIgnoreLoop) {
            candidateChildren0 = [];
            for (i = 0; i < candidateChildren.length; i++) if (!(candidateChildren[i].n.sign in visited)) candidateChildren0.push(candidateChildren[i]);
          ...
          child = bestChildren[Math.floor(Math.random() * bestChildren.length)];
      ...
      if (node == rootNode || node.visits >= uctParams.minVisitsExpand) {
        ...
        if (board.mFinished) {
          ...
          for (var i = 0; i < board.mMoves.length; i++) {
            ...
            for (var j = 0; j < signatures1.length; j++) aGame.RemoveVisit(null, signatures1[j]);
          }
          node.evaluation = bestEval;
          PropagateEval(node, uctParams.propagateMultiVisits ? board.mMoves.length : 1);
          if (uctParams.directVisits) for (var i = 0; i < nodePath.length; i++) nodePath[i].visits += uctParams.propagateMultiVisits ? board.mMoves.length : 1;
          if (known) {
            node.known = true;
            PropagateKnown(node);
          }
      ...
      if (uctParams.directVisits) for (var i = 0; i < nodePath.length; i++) nodePath[i].visits++;
    }
    var evalWeights = [];
    /*
    ...
    function UpdateDepthEval(evaluation, depth) {
      while (evalWeights.length <= depth) evalWeights.push({
        count: 0,
        sum: 0
      });
    ...
    if (!aGame.mBoard.mMoves || aGame.mBoard.mMoves.length == 0) aGame.mBoard.GenerateMoves(aGame);
    ...
    function Run() {
      ...
      if (!rootNode.children || rootNode.known == false && (uctParams.maxDuration <= 0 || now < uctParams.maxDuration * 1000 + t0) && (uctParams.maxLoops <= 0 || loopCount < uctParams.maxLoops) && (uctParams.maxNodes <= 0 || nodeCount < uctParams.maxNodes)) {
        do {
          ...
          for (var i = 0; i < signatures.length; i++) aGame.RemoveVisit(null, signatures[i]);
    ...
    function ReportStats(node) {
      ...
      if (uctParams.checkSide) {
        var _CheckSide = function CheckSide(node) {
          checkSideNodeCount++;
