...
} else (void 0).JoclyFairy = JoclyFairy;
(function () {
  ...
  function GetOrCreateWorker(aGame, aOptions) {
    ...
    if (typeof Worker == "undefined") throw new Error("fairy-stockfish: no Worker available in this environment (browser-only feature)");
    var worker = new Worker(baseURL + "jocly.fairyworker.js");
    var readyPromise = new Promise(function (resolve, reject) {
      worker.onmessage = function (e) {
        var message = e.data;
        if (message.type == "Ready") resolve();else if (message.type == "Error") reject(new Error(message.error));
      };
      worker.postMessage({
        type: "Init",
    ...
    if (workersByGame) workersByGame.set(aGame, entry);else fallbackWorkerSlot = {
      game: aGame,
      worker: entry
    };
    return entry;
  ...
  function BuildShogiStyleFen(aGame, dropPromoted) {
    ...
    for (var r = height - 1; r >= 0; r--) {
      ...
      for (var c = 0; c < width; c++) {
        var pos = r * width + c;
        if (!(pos in boardArea)) continue; // hand column for this row, skip entirely
    ...
    for (var i = 0; i < board.pieces.length; i++) {
      ...
      if (p.p < 0 || p.p in boardArea) continue;
      var pt = pieceTypes[p.t];
      if (!pt || pt.hand === undefined) continue; // not a real held piece (e.g. a "counter")
      var key = p.s + ":" + p.t;
    ...
    for (var key in counts) {
      ...
      if (!dropPromoted) letter = letter.replace(/^\+/, "");
      ...
      for (var i2 = 0; i2 < n; i2++) pocket += letter;
    }
  ...
  function BuildPieceMaps(pieceMap) {
    ...
    if (pieceMap) {
      for (var upper in pieceMap) {
        if (!pieceMap.hasOwnProperty(upper)) continue;
  ...
  function TranslitFen(fen, map) {
    if (!map || Object.keys(map).length === 0) return fen;
  ...
  function TranslitMove(uciMove, map) {
    if (!map || Object.keys(map).length === 0) return uciMove;
  ...
  function ResolveMove(aGame, uciMove, useChess960Format) {
    ...
    if (!candidates || candidates.length === 0) throw new Error("fairy-stockfish: no legal move available to match '" + uciMove + "'");
    ...
    if (bestIndex < 0) throw new Error("fairy-stockfish: could not match engine move '" + uciMove + "' to any legal move");
  ...
  function ResolveLevel(aGame, level) {
    if (!level.variants) return level;
    ...
    for (var i = 0; i < level.variants.length; i++) {
      if (level.variants[i].setup === setup) {
        ...
        // overriding them.
        var resolved = {};
        for (var k in level) if (level.hasOwnProperty(k) && k !== "variants") resolved[k] = level[k];
        for (var k2 in level.variants[i]) if (level.variants[i].hasOwnProperty(k2)) resolved[k2] = level.variants[i][k2];
        return resolved;
      }
    }
  ...
  JoclyFairy.startMachine = function (aGame, aOptions) {
    ...
    if (!variant) {
      if (level !== null)
        // null means ResolveLevel already logged a specific reason
    ...
    entry.ready.then(function () {
      return new Promise(function (resolve, reject) {
        entry.worker.onmessage = function (e) {
          ...
          switch (message.type) {
            case "Progress":
              if (aGame.mProgressCallback) aGame.mProgressCallback(message.percent);
              break;
            case "Done":
  ...
  JoclyFairy.abortMachine = function (aGame) {
    if (aGame.mFairyAbort) aGame.mFairyAbort();
  };
})();