...
function FairyLog() {
  if (typeof console !== "undefined" && console.info) console.info.apply(console, ["[fairy-stockfish]"].concat(Array.prototype.slice.call(arguments)));
}
function LoadEngine() {
...
function RunSearch(engine, options) {
  return new Promise(function (resolve, reject) {
    ...
    if (typeof options.skillLevel === "number") engine.postMessage("setoption name Skill Level value " + options.skillLevel);
    if (options.chess960) engine.postMessage("setoption name UCI_Chess960 value true");
    engine.postMessage("position fen " + options.fen);
    if (options.moveTimeMs) engine.postMessage("go movetime " + options.moveTimeMs);else engine.postMessage("go depth " + (options.depth || 12));
...
onmessage = function onmessage(e) {
  ...
  switch (message.type) {
    ...
    case "Search":
      ...
      break;
    case "Stop":
      if (RunSearch.currentStop) RunSearch.currentStop();
      break;
  }
};