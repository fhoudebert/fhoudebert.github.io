...
function ScanLog() {
  if (typeof console !== "undefined" && console.info) console.info.apply(console, ["[scan]"].concat(Array.prototype.slice.call(arguments)));
}
...
function LoadEngine() {
  if (scanEngineReady) return scanEngineReady;
  ...
  scanEngineReady = FetchArrayBuffer(scanBaseURL + "scan.wasm").then(function (wasmBinary) {
    return ScanModule({
    ...
    return FetchArrayBuffer(scanBaseURL + "data/eval").then(function (evalBuf) {
      ...
      var ok = mod.ccall("scan_init", "number", ["number", "number"], [scanBookLoaded ? 1 : 0, TT_SIZE_LOG2]);
...
function RunSearch(mod, options) {
  ...
  if (!okPos) throw new Error("scan: invalid FEN '" + options.fen + "'");
  ...
  var move = mod.ccall("scan_go", "string", ["number", "number", "number"], [timeSec, depthMax, useBook ? 1 : 0]);
  ...
  switch (message.type) {
    ...
    case "Search":
      ...
      LoadEngine().then(function (mod) {
        if (!searchPending) throw {
          aborted: true
        }; // Stop arrived before the engine finished loading
        ...
        searchPending = false;
        if (err && err.aborted) postMessage({
          type: "Aborted"
        });else postMessage({
          type: "Error",
